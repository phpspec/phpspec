...
class BeString implements Matcher
{
     ...
     * but beString() is itself the expectation
     * 
    ...
    /**
     * Checks whether actual value is string
     * 
     * @param mixed $actual
     * @return boolean
     */
    ...
    {
        $type = is_object($this->_actual) ? get_class($this->_actual) :
                gettype($this->_actual);
        return 'expected to be string, got ' . var_export($this->_actual, true)
               . ' type of ' . $type . ' (using beString())';
    }
    ...
    public function getNegativeFailureMessage()
    {
        return 'expected ' . var_export($this->_actual, true) .
               ' not to be string (using beString())';
    }
