Feature: Paths can be relative to config or current working directory
    ...
    Scenario: Config option "use_config_path" is not set
        Given the config file located in "Awesome" contains:
            """
            suites:
              behat_suite:
                namespace: MilkyWay\OrionCygnusArm
            """
        And I have started describing the "MilkyWay/OrionCygnusArm/DumbbellNebula" class with the "Awesome/phpspec.yml" custom config
        When I run phpspec with the "Awesome/phpspec.yml" custom config and answer "y" when asked if I want to generate the code
        Then a new class should be generated in the "src/MilkyWay/OrionCygnusArm/DumbbellNebula.php":
            """
            <?php

            namespace MilkyWay\OrionCygnusArm;

            class DumbbellNebula
            {
            }
