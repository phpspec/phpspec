Feature: Guard
  ...
  Scenario: Reaching part of a change is said differently from reaching none of it
    Given a PSR-4 project with "spec" and "src" directories
    And a class "src/App/Basket.php":
      """
      <?php
    ...
    And a spec file "spec/App/Basket.spec.php":
      ...
      """
    And I run phpspec run with coverage options ""
    Then the output should contain "is still unreached"
    And the output should contain "Extend your examples"
    And the output should not contain "then make it pass"
    And the exit code should be 1
